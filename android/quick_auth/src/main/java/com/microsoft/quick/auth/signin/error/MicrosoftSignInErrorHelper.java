...
public class MicrosoftSignInErrorHelper {
    /**
    ...
    public static MicrosoftSignInError convertToSignError(@NonNull Exception e) {
        MicrosoftSignInError error;
        if (e instanceof MsalException) {
            error = new MicrosoftSignInError(((MsalException) e).getErrorCode(), e.getMessage());
        } else if (e instanceof InterruptedException) {
            error = new MicrosoftSignInError(INTERRUPTED_ERROR, e.getMessage());
        } else {
            error = new MicrosoftSignInError(UNEXPECTED_ERROR, e.getMessage());
        }
        error.addSuppressedException(e);
        return error;
    }

    public static <T> T convertToSignError(ErrorConvertFunction<T> function) throws MicrosoftSignInError {
        try {
            return function.run();
        } catch (MicrosoftSignInError e) {
            throw e;
