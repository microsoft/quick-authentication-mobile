...
public class TaskThen<T, R> extends Task<R> {
  ...
  private final @NonNull Task<T> mSource;
  private final @NonNull Convert<? super T, ? extends Task<? extends R>> mConverter;

  public TaskThen(
      @NonNull Task<T> source, @NonNull Convert<? super T, ? extends Task<? extends R>> converter) {
    this.mSource = source;
  ...
  static class MapConsumer<T, R> implements Consumer<T> {
    private final @NonNull Consumer<? super R> mDownStream;
    private final @NonNull Convert<? super T, ? extends Task<? extends R>> mConverter;
    private Disposable mDisposable;

    public MapConsumer(
        @NonNull Consumer<? super R> consumer,
        @NonNull Convert<? super T, ? extends Task<? extends R>> convert) {
